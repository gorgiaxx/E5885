...
#else
UDI_HANDLE BSP_UDI_NCM_DataChnToHandle(u32 u32Chn)
{
    return (UDI_HANDLE)(u32Chn);
}
#endif
...
/*****************************************************************************
* 函 数 名  : ncm_get_ctx_ex
*
* 功能描述  : 根据设备ID获取上下文
*
* 输入参数  :u32 config     :对应config值
*          *: u32 dev_id    :设备ID
* 输出参数  : 无
*
* 返 回 值  : 设备ID上下文
...
*
*****************************************************************************/
static inline ncm_app_ctx_t *ncm_get_ctx_ex(u32 config, u32 dev_id)
{
    //DBG_I(MBB_NCM, "[%d] config = %d\n",__LINE__,config);//config 值由调用处保证
    if ((USB_NUM_1 <= config) && (USB_NUM_3 >= config))
    {    config--;}
    else
    {    config = 0;}
    return (&ncm_app_ctx_set[config][dev_id]);
...
* 返 回 值  : NA
*
* 其它说明  : by ncm_vendor open close write ioctl called
*
*****************************************************************************/
...
* 其它      : NA
*****************************************************************************/
void ncm_vendor_close_ex(NCM_DEV_TYPE_E dev_type, u32 dev_id)
{
    ncm_get_ctx_ex(USB_NUM_1, dev_id)->chnl_open[dev_type] = false;
    ncm_get_ctx_ex(USB_NUM_2, dev_id)->chnl_open[dev_type] = false;
    ncm_get_ctx_ex(USB_NUM_3, dev_id)->chnl_open[dev_type] = false;
}
extern void ecm_vendor_close_ex(NCM_DEV_TYPE_E dev_type, u32 dev_id);
s32 ncm_vendor_close(NCM_DEV_TYPE_E dev_type, u32 dev_id)
{
    ...
    ncm_vendor_close_ex(dev_type, dev_id);
    ecm_vendor_close_ex(dev_type, dev_id);
...
s32 ncm_vendor_write(u32 dev_id, void *pkt_encap, void *net_priv)
{
    ...
    if (app_ctx->vendor_ctx->is_mbim_mode)
    {
        ...
        /* Ip header 为0的ARP包，在MBIM模式下需要丢弃 */
...
void setup_app_to_function_connect(ncm_vendor_ctx_t *vendor_ctx)
{
    ncm_app_ctx_t *app_ctx = NULL;
...
s32 ncm_vendor_init(ncm_vendor_ctx_t *vendor_ctx)
{
    ...
    DBG_I(MBB_NCM, "%s get idel dev id %d\n", __FUNCTION__, dev_id);
