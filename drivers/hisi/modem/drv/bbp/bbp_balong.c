...
void balong_bbp_get_om_info(int print_enable)
{
    ...
    printk(KERN_ERR"balong_bbp_dump_hook in...paddr = %llu\n", (u64)virt_to_phys(g_bbp_acore_om_info_addr));
    ...
    for (i = BBP_PCTRL_OM_CTRL_CMD_MIN; i <= BBP_PCTRL_OM_CTRL_CMD_MAX; i++) {
        ...
        if (!g_bbp_acore_om_info_addr) {
            writel(value, g_bbp_acore_om_info_addr + (i - BBP_PCTRL_OM_CTRL_CMD_MIN)*4);
        }
...
static int __init balong_bbp_acore_init(void)
{
    ...
    g_bbp_acore_om_info_addr = bsp_pm_dump_get(PM_OM_BBP, ((BBP_PCTRL_OM_CTRL_CMD_MAX - BBP_PCTRL_OM_CTRL_CMD_MIN) + 1) * 4);
